...
class MAClsPredictor:
    ...
    def predict(self,
        ...
        # with open(path) as f:
        #     f = open(path)
        #     audios_data = f.read().splitlines()
        # text = []
        #
        # for audio_data in audios_data:
        #     audio_data = audio_data
        #     text.append(audio_data)
    ...
    """
        ...
        for audio_data in audios_data:
            ...
            text.append(audio_data)
           # (filepath, filename) = os.path.split(audio_data)
           # print(filepath)
        # 找出音频长度最长的
        ...
        # for audio_data in audios_data:
        #     audio_data = audio_data
        return text,labels, scores

    # def que(self, que, predict):
    #
    #     path = que
    #
    #     with open(path) as f:
    ...
    # def predict_batch_new(self,
    #                   audios_data:List,
    #                   sample_rate=16000):
    #     """预测一批音频
    #
    #     :param audio_paths: 需要识别的音频路径列表
    #     :param sample_rate: 如果传入的事numpy数据，需要指定采样率
    #     :return: 预测结果列表，每个元素是一个三元组，包含音频路径、结果标签和对应的得分
    #     """
    #     path = audios_data
    #
    #     with open(path) as f:
    ...
    #         lab = np.argsort(result)[-1]
    #         score = result[lab]
    #         results.append((audio_data, self.class_labels[lab], round(float(score), 5)))
    #     return results

    # def read_audio_paths_from_txt(txt_file_path: str):
    #     """
    #     从txt文件中读取音频文件路径列表
    ...
    #
    # # 读取txt文件中的音频文件路径列表
    # audio_paths = read_audio_paths_from_txt(audio_paths)
    # # 预测所有音频文件并输出结果
    # results = predict_batch_new(audio_paths)
    # for result in results:
    #     print(result)